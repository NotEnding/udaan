...
class DbService:
    ...
    def __init__(self):
        ...
        self.listing_collection = self.db[mongo_dict['listing_collection']]  # 商品表
        self.org_collection = self.db[mongo_dict['org_collection']]  # 店铺表
        self.categories_collection = self.db[mongo_dict['categories_collection']]  # 品类表
        self.best_seller_collection = self.db[mongo_dict['best_seller_collection']] # 卖得最好表
        self.fast_moving_collection = self.db[mongo_dict['fast_moving_collection']] # 最快增长表
        self.latest_arrival_collection = self.db[mongo_dict['latest_arrival_collection']] #最新到货表
        # redis配置
        redis_config = {
